...
# calculate tiles of chessboard with corners
def getTiles(corners): # corners = 9x9x2 matrix
    tiles = np.zeros((8,8), dtype=Tile)
    for row in range(0, 8):
        for column in range(0, 8):
            tiles[row][column] = Tile(corners[row][column],
...
def setup(image):
    ...
    image = cv2.resize(image, (416,416), interpolation=cv2.INTER_AREA) # resize image to 416x416
    im_bw = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
    (found, corners) = cv2.findChessboardCorners(im_bw, (7, 7), None) #inner corners
    if found:
        boardMatrix = getAllOuterPoints(np.reshape(corners, (7, 7, 2)), image)
        chessboard.tiles = getTiles(boardMatrix)
        return chessboard
    else:
